...
const QUESTION_START = /^(?:should|shall|what|which|where|when|who|do|does|would|want|let'?s|vote|poll|co|kam|kdy|kdo|který|která|které|jaký|jaká|jaké|jakou|půjdeme|pujdeme|dáme|dame|chcete|chceš|hlasujte|anketa)(?![\p{L}])/iu;
const CONTEXT = /\s+((?:for|on|at|this|next|tonight|tomorrow|today|na|v|ve|pro|dnes|zítra|večer|tento|tuto|příští)(?=\s|$).*)$/i;
...
export function parsePoll(text: string): PollData {
  ...
  let parts = t.split(/\s*(?:,|\bor\b|\bnebo\b|\bvs\.?\b|\/)\s*/i).filter(Boolean);
  ...
  if (stem && /^(?:pick|choose)(?: one)?$/i.test(stem)) stem = "Vyber jedno";
  parts = parts.map((p) => p.trim()).filter(Boolean);
  const base = capitalize(stem ?? parts.join(" nebo "));
