...
export function normalizeExpression(text: string): string {
  ...
  s = s.replace(/^(?:what(?:'s| is)|calc(?:ulate)?|compute|how much is|kolik (?:je|dělá|dela|bude)|(?:spočítej|spocitej|vypočítej|vypocitej|spočti|spocti)(?: mi)?)\s+/, "").replace(/[=?]+\s*$/, "");
  // Czech numbers: "2 400" and "12,5"
  s = s.replace(/(\d) (\d{3})(?!\d)/g, "$1$2").replace(/(\d),(\d{1,2})(?!\d)/g, "$1.$2");
  s = s.replace(/(\d+(?:\.\d+)?)\s*(?:%|procent|procenta)\s+(?:z|ze)\s+/g, "$1% of ").replace(/\s*(?:procent|procenta)\b/g, "%");
  s = s.replace(/\bsleva\s+(\d+(?:\.\d+)?)\s*%\s+(?:z|ze|na)\s+/g, "$1% off ");
