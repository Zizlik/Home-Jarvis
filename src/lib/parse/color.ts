...
/** Czech color stems (adjective endings stripped) → English names above. */
const CZECH_COLORS: Record<string, string> = {
  cerven: "red", rud: "red", karminov: "crimson", sarlatov: "scarlet", vinov: "wine", bordov: "burgundy",
  ruzov: "pink", korálov: "coral", koralov: "coral", lososov: "salmon", broskvov: "peach", oranzov: "orange",
  mandarinkov: "tangerine", jantarov: "amber", zlat: "gold", zlut: "yellow", horcicov: "mustard", citronov: "lemon",
  kremov: "cream", bezov: "beige", piskov: "sand", hned: "brown", cokoladov: "chocolate", olivov: "olive",
  limetkov: "lime", zelen: "green", salvejov: "sage", matov: "mint", smaragdov: "emerald", lesni: "forest",
  tyrkysov: "turquoise", azurov: "sky", blankytn: "sky", nebesk: "sky", modr: "blue", namornick: "navy",
  kobaltov: "cobalt", indigov: "indigo", fialov: "purple", levandulov: "lavender", seribov: "lilac", purpurov: "magenta",
  svestkov: "plum", sed: "grey", sediv: "grey", cern: "black", bil: "white", slonovinov: "ivory",
  stribrn: "silver", medeny: "copper", meden: "copper", bronzov: "bronze", terakotov: "terracotta", rezav: "rust",
  kavov: "coffee", tyrkys: "turquoise", petrolejov: "teal", khaki: "olive", lilaković: "lilac",
};
const plainCs = (s: string) => s.normalize("NFD").replace(/\p{M}/gu, "").toLowerCase();
function czechColor(word: string): string | null {
  const p = plainCs(word);
  const stem = p.replace(/(?:ymi|ych|ymi|eho|emu|ou|a|y|e|i|u|o)$/u, "");
  return CZECH_COLORS[stem] ?? CZECH_COLORS[p] ?? null;
}

function modifiers(text: string, hex: string) {
  const o = hexToOklch(hex);
  if (!o) return hex;
  if (/(?<![\p{L}])(light|pale|soft|pastel|baby|světl\p{L}*|svetl\p{L}*|bled\p{L}*|pastelov\p{L}*|jemn\p{L}*)(?![\p{L}])/iu.test(text)) return oklchToHex({ l: Math.max(o.l, 0.86), c: o.c * 0.55, h: o.h });
  if (/(?<![\p{L}])(dark|deep|midnight|tmav\p{L}*|temn\p{L}*|hlubok\p{L}*)(?![\p{L}])/iu.test(text)) return oklchToHex({ l: Math.min(o.l, 0.38), c: o.c * 0.85, h: o.h });
  if (/(?<![\p{L}])(bright|neon|vivid|electric|zářiv\p{L}*|zariv\p{L}*|neonov\p{L}*|syt\p{L}*|jasn\p{L}*)(?![\p{L}])/iu.test(text)) return oklchToHex({ l: o.l, c: o.c * 1.25, h: o.h });
  if (/(?<![\p{L}])(muted|dusty|faded|matn\p{L}*|zašl\p{L}*|zasl\p{L}*|tlumen\p{L}*)(?![\p{L}])/iu.test(text)) return oklchToHex({ l: o.l, c: o.c * 0.5, h: o.h });
  return hex;
}
...
export function parseColor(text: string, mood?: ColorMood | null): ColorData {
  ...
  for (let i = words.length - 1; i >= 0; i--) {
    ...
    const cs = czechColor(w);
    const csHex = cs ? (NAMED_COLORS[cs] ?? EVOCATIVE_COLORS[cs] ?? REFERENCE_COLORS[cs]) : undefined;
    if (csHex) {
      // Show the Czech word as typed ("tyrkysová"), color it like its English twin.
      return { hex: modifiers(text, csHex), name: w, source: "named" };
    }
  }
