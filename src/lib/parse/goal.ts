...
export function parseGoal(text: string): GoalData {
  ...
  const unitMatch = rest.match(/^\s*(?:\p{L}+\s+)?(books?|km|kms|miles?|pages?|workouts?|runs?|steps?|kg|lbs?|hours?|articles?|courses?|₹|rs|\$|dollars|rupees|kn(?:iha|ihy|ih)|strán(?:ka|ky|ek)|kilometr(?:ů|y)?|tréninky?|tréninků|běhy?|běhů|kroky?|kroků|hodiny?|hodin|články?|článků|kurzy?|kurzů|kč|korun)(?=\s|$|,)/iu);
