...
export function parseContact(text: string): ContactData {
  ...
  const name = titleCase(
    rest
      .replace(/(?<![\p{L}])(?:save|add|contact|number|phone|email|mail|is|his|her|their|new|ulož|uloz|přidej|pridej|kontakt|číslo|cislo|telefon|tel|mobil|e-mail|je|jeho|její|nový|nova|nový kontakt)(?![\p{L}])|:/giu, " ")
      .replace(/[^\p{L}\s'.-]/gu, " ")
      .trim(),
  );
  const initials = name
