...
const NUMBER_LIST: [string, number][] = [
  ...
  ["jedenáct|jedenácti", 11], ["dvanáct|dvanácti", 12], ["třináct|třinácti", 13], ["čtrnáct|čtrnácti", 14], ["patnáct|patnácti", 15],
  ["šestnáct|šestnácti", 16], ["sedmnáct|sedmnácti", 17], ["osmnáct|osmnácti", 18], ["devatenáct|devatenácti", 19], ["dvacet|dvaceti", 20],
  ["třicet|třiceti", 30], ["čtyřicet|čtyřiceti", 40], ["padesát|padesáti", 50], ["šedesát|šedesáti", 60], ["sedmdesát|sedmdesáti", 70],
  ["osmdesát|osmdesáti", 80], ["devadesát|devadesáti", 90], ["sto|sta", 100],
];
const TENS = "dvacet|dvaceti|třicet|třiceti|čtyřicet|čtyřiceti|padesát|padesáti|šedesát|šedesáti|sedmdesát|sedmdesáti|osmdesát|osmdesáti|devadesát|devadesáti";
...
function numbers(text: string) {
  return apply(text, [
    ...
    // after the number words, so "dvaceti tisíc" → "20k"
...
/** Calc reads Czech itself (lib/parse/calc.ts); it only needs the numbers in digits. */
const calc = (t: string) => t;
...
export const TYPED_TEXT: ReadonlySet<CardIntent> = new Set(["poll", "note", "link", "contact", "color"]);
